...
>
    <div class={['group flex flex-row items-center gap-3 text-black sm:gap-4', expanded ? 'text-accent' : 'text-black']}>
        ...
        <div class="flex flex-col space-y-1 text-left transition">
            <p class="text-accent text-3xl leading-[1em] font-black md:text-left md:text-4xl">{member.username}</p>
            <p class="text-xl leading-[1em] sm:text-2xl md:text-left">{member.realName}</p>
        </div>
